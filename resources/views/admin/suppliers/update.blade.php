...
<div class="wrapper">
        ...
        <div class="content-wrapper iframe-mode" data-widget="iframe" data-loading-screen="750">
            ...
            <form style="padding: 20px 50px;" action="{{ route('suppliers.update', ['supplier' => $editSuppliers->id]) }}" method="post">
                @csrf
                @method('PUT')
                <div class="mb-3">
                    <label for="exampleFormControlInput1" class="form-label">Tên nhà cung cấp</label>
                    <input type="text" name="name" class="form-control" id="exampleFormControlInput1" placeholder="nhập tên sản phẩm.." value="{{$editSuppliers->name}}">
                </div>
                @error('name')
                ...
                <div class="mb-3">
                    ...
                    <input type="text" name="slug" class="form-control" id="exampleFormControlInput1" placeholder="nhập size sản phẩm.." value="{{$editSuppliers->slug}}">
                </div>
                @error('slug')
                <div class="col-3 alert alert-info">{{ Str::limit($message, 50) }}</div>
                @enderror
